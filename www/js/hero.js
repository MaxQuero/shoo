...
Hero.prototype.setXY = function(x, y) {
    this.x = x;
    this.y = y;
};
/**
 ...
 */
Hero.prototype.renderTo = function( context )
{
    context.save();
    context.translate( this.x, this.y )
    context.drawImage( Hero.img, 0, 0, 100, 100 );
    context.restore();
}
 ...
 */
Hero.prototype.hits = function( x, y )
{
    return x >= this.x
        && x <= this.x + this.size
        && y >= this.y
 ...
 */
Hero.prototype.move = function( bounds )
{
    this.x += this.vx;
    this.y += this.vy;
    if ( this.x < 0 )
